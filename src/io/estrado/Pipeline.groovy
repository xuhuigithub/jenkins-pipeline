...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        sh "helm upgrade --dry-run --install ${args.name} ${args.chart_dir} --set Huiblog.imageTag=${args.version_tag},Huiblog.replicas=${args.replicas},Huiblog.cpu=${args.cpu},Huiblog.memory=${args.memory},ingress.hostname=${args.hostname} --namespace=${namespace}"
    } else {
        println "Running deployment"

        // reimplement --wait once it works reliable
        sh "helm upgrade --install ${args.name} ${args.chart_dir} --set Huiblog.imageTag=${args.version_tag},Huiblog.replicas=${args.replicas},Huiblog.cpu=${args.cpu},Huiblog.memory=${args.memory},ingress.hostname=${args.hostname} --namespace=${namespace}"
...
def containerBuildPub(Map args) {
    ...
    docker.withRegistry("https://${args.host}", "${args.auth_id}") {
        ...
        sh "docker build --build-arg VCS_REF=${env.GIT_SHA} --cache-from https://${args.host}/${args.acct}/${args.repo}:latest --build-arg BUILD_DATE=`date -u +'%Y-%m-%dT%H:%M:%SZ'` -t ${args.acct}/${args.repo} ${args.dockerfile}"
